...
static inline void terminal_writePrefix(terminal_handle_t* pHandle){
	...
	pHandle->Abstract.Xfer.sendString("-> \0");
}
...
static void terminal_clearReadCache(terminal_handle_t* pHandle){
	char ch;
	while(1){
	...
	terminal_insertCharToBuffer(pHandle, ch);
}
...
bool terminal_start(terminal_handle_t* pHandle){
	...
	terminal_clearReadCache(pHandle);
